...
# --- CORREÇÃO DA LOGICA DE PREENCHIMENTO DE PLACAR ---
def disparar_atualizacao_placar(m_str, j1, j2):
    ...
    # REGRA CRÍTICA CORRIGIDA: Só trava em 72 se for EXATAMENTE 2x0 seco.
    if s1 == 2 and s2 == 0:
        t1 = 72
        t2 = st.session_state.get(f"dir_t2_{m_str}_r{sem}", p_antigo[3])
        if t2 > 46: t2 = 46
    elif s2 == 2 and s1 == 0:
        t2 = 72
        t1 = st.session_state.get(f"dir_t1_{m_str}_r{sem}", p_antigo[2])
        if t1 > 46: t1 = 46
    else:
        # Se for 2x1, os dois campos ficam livres para digitação manual
        t1 = st.session_state.get(f"dir_t1_{m_str}_r{sem}", p_antigo[2])
        t2 = st.session_state.get(f"dir_t2_{m_str}_r{sem}", p_antigo[3])
...
# --- AUXILIAR DE PREENCHIMENTO INTELIGENTE DE MESA CORRIGIDO ---
def renderizar_formulario_mesa_admin(m, j1, j2, sem_id):
    ...
    with c2:
        if not jogo_encerrado:
            ...
            # CASO A: Vencedor por 2x0 seco (Jogador 1)
            if s1_in == 2 and s2_in == 0:
                st.info(f"💡 {j1} venceu por 2x0. Tentos travados em 72.")
                st.number_input(f"Tentos - {j1}", 72, 72, 72, key=f"dir_t1_{m}_r{sem_id}", disabled=True)
                st.number_input(f"Tentos - {j2} (Máx: 46)", 0, 46, min(int(t2), 46), key=f"dir_t2_{m}_r{sem_id}", on_change=disparar_atualizacao_placar, args=(m, j1, j2))
            
            # CASO B: Vencedor por 2x0 seco (Jogador 2)
            elif s2_in == 2 and s1_in == 0:
                st.number_input(f"Tentos - {j1} (Máx: 46)", 0, 46, min(int(t1), 46), key=f"dir_t1_{m}_r{sem_id}", on_change=disparar_atualizacao_placar, args=(m, j1, j2))
                st.info(f"💡 {j2} venceu por 2x0. Tentos travados em 72.")
                st.number_input(f"Tentos - {j2}", 72, 72, 72, key=f"dir_t2_{m}_r{sem_id}", disabled=True)
                
            # CASO C: Placar equilibrado de 2x1 (Libera digitação total, sem travar 72)
            else:
                # Garante que se o valor antigo guardava 72 de um 2x0 anterior, ele resete para um número válido digitável
                t1_def = 48 if int(t1) == 72 else int(t1)
                t2_def = 24 if int(t2) == 0 else int(t2)
                
                st.number_input(f"Tentos - {j1}", 0, 72, t1_def, key=f"dir_t1_{m}_r{sem_id}", on_change=disparar_atualizacao_placar, args=(m, j1, j2))
                st.number_input(f"Tentos - {j2}", 0, 72, t2_def, key=f"dir_t2_{m}_r{sem_id}", on_change=disparar_atualizacao_placar, args=(m, j1, j2))
